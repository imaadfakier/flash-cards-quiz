...
def word_is_known():
    ...
    for same_dict in language_data_dict:
        if known_word != same_dict[language_data.columns[0]]:
            continue
        language_data_dict.remove(same_dict)
        break

    words_to_learn_df = pandas.DataFrame(data=language_data_dict)
    words_to_learn_df.to_csv(path_or_buf='./data./words_to_learn.csv', index=False)
...
#
# try:
#     saved_language_data = pandas.read_csv(filepath_or_buffer='./data/words_to_learn.csv')
# except FileNotFoundError:
#     original_language_data = pandas.read_csv(filepath_or_buffer='./data/french_words.csv')
#     a_language_data_dict = original_language_data.to_dict(orient='records')
# else:
#     saved_language_data_dict = saved_language_data.to_dict(orient='records')
# finally:
#     new_random_word()
